...
**⚠️ 关键：每次调度 agent 前必须执行以下步骤：**

1. **提取关键词**：从用户请求中提取核心技术关键词
2. **自动召回**：执行 `node scripts/embedding/kg-recall-module.js "<关键词>"`
3. **注入上下文**：将召回结果**追加到任务描述开头**

**示例 - 用户说 "帮我修复openclaw项目的数据丢失问题"：**

```
任务：修复 openclaw 项目的数据丢失问题

📚 **相关记忆** (数据 丢失 修复):
1. [session] 会话 xxx (65.3%)
2. [tool] sessions_list (42.1%)
3. [tool] message (38.5%)
...
**注意：**
- ✅ 每次调度**都必须**执行召回（除简单问候外）
- ✅ 召回结果作为任务背景，帮助 agent 了解相关上下文
- ✅ 调用命令已简化：`node scripts/embedding/kg-recall-module.js "关键词"`
